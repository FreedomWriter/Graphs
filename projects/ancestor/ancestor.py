def earliest_ancestor(ancestors, starting_node):   
    ...
    # list for storing all possible solutions, 
    solution_list = []

    # iterate through the ancestors list
    for i in range(len(ancestors)):
        # append each el in tupple to solution_list
        solution_list.append(ancestors[i][0])
        solution_list.append(ancestors[i][1])
        # populate hash_table where the key is the child, and the value is an array of all of the childs parents
        if not ancestors[i][1] in hash_table:
            hash_table[ancestors[i][1]] = [ancestors[i][0]]
        else:
            hash_table[ancestors[i][1]].append(ancestors[i][0])
    ...
    while iterating:
        ...
        if solution in hash_table:
            solution = hash_table[solution][0] # zeroeth index stores the earlier of the ancestors when there are two
        # if the value stored in solution is not in the hash_table, stop iterating
        else:
            iterating = False
    # if the solution is in the solution_list and it is not the starting_node, return the solution, otherwise return -1
    if solution != starting_node and solution in solution_list :
        return solution
    else:
        return -1
